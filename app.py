...
CLIP_GDRIVE = r"G:\Il mio Drive\CLIP_MYGAME"
...
def invia_conferma_e_salva(email_utente):
    ...
    msg = MIMEText(f"Grazie per esserti iscritto alla newsletter di Face Soccer - MYGAME!")
    msg['Subject'] = "Conferma Iscrizione MyGame"
...
# --- STILE E CSS ---
st.set_page_config(page_title="Face Soccer - MYGAME", layout="wide")
st.markdown("""
    <style>
...
if st.session_state.autenticato:
    is_admin = st.session_state.user_email == "admin@mygame.com"
...
if st.session_state.pagina == 'home' and not st.session_state.autenticato:
    st.markdown("<h1 style='text-align: center;'>⚽ FACE SOCCER - MYGAME</h1>", unsafe_allow_html=True)
...
# --- LOGIN ---
    ...
    with col_log:
        ...
        if st.button("ENTRA"):
            ...
            conn.close()
            if (u == "admin@mygame.com" and p == "admin123") or user:
...
st.markdown("<br><hr><p class='footer-main'>facesoccer- mygame</p>", unsafe_allow_html=True)
