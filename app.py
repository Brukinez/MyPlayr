    ...
    try:
        ...
        if user:
            ...
            with cr:
                ...
                st.markdown(f'<div style="background:#262730; padding:20px; border-radius:10px;"><b>Bio:</b> {user.get("bio") or "Nessuna bio"}</div>', unsafe_allow_html=True)
            ...
            for i, (ico, tit, val) in enumerate(stats):
                with s_cols[i]: 
                    st.markdown(f'<div style="text-align:center; background:#1E1E1E; padding:40px; border-radius:10px; border:1px solid #28a745;">{ico}<br><small>{tit}</small><br><b>{val}</b></div>', unsafe_allow_html=True)
